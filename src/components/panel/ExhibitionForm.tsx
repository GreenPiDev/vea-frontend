...
export default function ExhibitionForm({ onDone }: ExhibitionFormProps) {
  ...
  function toggleCell(x: number, z: number, value: boolean) {
    ...
    setCells((prev) => {
      ...
      if (spawnCell && !value && spawnCell.x === x && spawnCell.z === z) setSpawnCell(null);
      return next;
    });
  }
  ...
  const canSubmitCustom = roomType !== 'custom' || (cells.size > 0 && spawnCell != null);
            ...
            {Array.from({ length: gridRows }).map((_, z) =>
              Array.from({ length: gridCols }).map((_, x) => {
                ...
                return (
                  <div
                    key={`${x},${z}`}
                    className={`exform-cell ${pickingSpawn && filled ? 'pickable' : ''}`}
